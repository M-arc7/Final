# ACCERA Caching Architecture

> **Status:** Baseline controlled specification. Update this document in the same change as any contract it governs.

## Purpose and boundary

Client, server, API, database and CDN caching, invalidation, TTLs and consistency requirements.

Cache only derived/read data; mutations invalidate by domain event or version. Never cache authorization decisions beyond the current request/session policy.
